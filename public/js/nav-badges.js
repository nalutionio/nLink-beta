(() => {
  ...
  const loadProviderProposalNotificationTokens = async (providerId) => {
    ...
    return Array.isArray(data)
      ? data
        .filter((row) => row?.id)
        .map((row) => `${String(row.id)}:${String(row.status || "requested")}`)
      : [];
  };
  ...
  const init = async () => {
    ...
    if (isProvider) {
      ...
      const refreshProviderBadges = async () => {
        ...
        const acceptedTokens = await loadProviderProposalNotificationTokens(providerId);
