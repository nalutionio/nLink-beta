...
-- job reviews: reviewee can read own, provider reviews are marketplace-readable.
-- inserts constrained to valid client/provider relationships on completed jobs.
do $$
begin
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public' and tablename = 'job_reviews' and policyname = 'job_reviews_select_policy'
  ) then
  ...
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public' and tablename = 'job_reviews' and policyname = 'job_reviews_insert_policy'
  ) then
